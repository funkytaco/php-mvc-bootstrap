...
class ApplicationTasks {
				...
				if ( is_array( $extra ) ) {
					...
					if ( array_key_exists( $strAssetKey, $extra ) ) {
						...
						echo self::ansiFormat(
							'ERROR',
							'Invalid asset key (' . $strAssetKey . '). Check the extras section of your composer.json file
	...
	public static function InstallMvc( Event $event ) {
		if ( ! self::AreComposerPackagesInstalled( $event ) ) {
		...
		if ( $isFile == true ) {
			...
			try {
				...
				if ( self::copy_assets_recursive( $source, $destination, $event ) == true ) {
					...
					echo self::ansiFormat(
						'ERROR',
						'Copy failed! Unable to copy assets from "' . realpath( $source )
